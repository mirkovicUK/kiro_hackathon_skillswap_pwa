...
- Demo video narration generated using AWS Polly (Kiro wrote its own voice script!)
...
6. **Demo Video** - Voice script and audio generation with AWS Polly
...
- Kiro can generate its own presentation materials (voice scripts, audio)
...
- **Meta-innovation**: Kiro gives itself a voice by generating the demo narration script

---

### Phase 17: Demo Video Generation [~1h]

**Date**: January 16, 2026

**Completed**:
- Created complete spec for demo video assets (`.kiro/specs/demo-video/`)
- Generated voice script optimized for Amazon Polly with SSML markup
- Built Python audio generator using AWS Polly API

**Files Created**:
| File | Purpose |
|------|---------|
| `video/demo_script.txt` | SSML-formatted voice script (~3 minutes) |
| `video/generate_audio.py` | Python script using boto3 + AWS Polly |
| `video/demo_audio.mp3` | Generated audio narration |

**Voice Script Sections**:
| Section | Content | Duration |
|---------|---------|----------|
| INTRO | Value proposition, urban problem, triple-solve | ~30s |
| KIRO_USAGE | 5 specs, 7 steering docs, 45 properties | ~25s |
| HOMEPAGE | Landing page walkthrough | ~15s |
| AUTH | Registration and login | ~15s |
| APP_FLOW | Profile → Discover → Match → Meet → Confirm | ~60s |
| OUTRO | Testing instructions, closing | ~20s |

**Python Generator Features**:
- Configurable voice at top of script (`VOICE_ID = 'Matthew'`)
- SSML parsing and validation
- Automatic text splitting for Polly's 3000 character limit
- Neural engine with standard fallback
- MP3 segment concatenation
- Clear progress output and error handling

**Supported Voices** (Standard + Neural):
- Matthew (male, news narrator) ✅
- Joanna (female, professional) ✅
- Joey (male, young) ✅
- Kendra (female, warm) ✅
- Kimberly (female, professional) ✅
- Salli (female, soft) ✅

**Meta-Innovation**: Kiro wrote its own demo presentation script, then generated the audio narration using AWS Polly. The AI literally gave itself a voice for the hackathon submission.

**Usage**:
```bash
# Edit VOICE_ID in video/generate_audio.py if desired
python3 video/generate_audio.py
# Output: video/demo_audio.mp3
```
...
|----------|--------|---------|
| Application Quality | ✅ Complete | Full user flow working |
| Kiro IDE Usage | ✅ Complete | 6 specs, 7 steering docs, 45 properties |
| Documentation | ✅ Complete | README, DEVLOG, Architecture docs |
| Innovation | ✅ Complete | Triple-solve, mandatory coffee meeting |
| Demo Video | ✅ Complete | Audio generated with AWS Polly |

**Final Stats**:
- 6 feature specs with requirements, design, and tasks
- 7 steering documents providing AI context
- 45 correctness properties tested with fast-check
- 13 custom prompts for workflow automation
- ~17 hours total development time
- Demo video narration generated by Kiro using AWS Polly
